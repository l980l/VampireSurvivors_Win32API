...
CWhiteBat::CWhiteBat(const CWhiteBat& Obj) :
	CMonster(Obj),
	m_Dir(Obj.m_Dir),
	m_HP(Obj.m_HP),
...
bool CWhiteBat::Init()
{
	...
	AddAnimation("WhiteBat");
	AddAnimation("WhiteBatDeath", false, 0.3f);
	AddAnimation("DamagedBat2", false, 0.3f);
	AddAnimation("WhiteBatRight");
	AddAnimation("WhiteBatDeathRight", false, 0.3f);
	AddAnimation("DamagedBat2Right", false, 0.3f);

	m_vecSequenceKey[0].push_back("WhiteBat");
	m_vecSequenceKey[0].push_back("WhiteBatDeath");
	m_vecSequenceKey[0].push_back("DamagedBat2");

	m_vecSequenceKey[1].push_back("WhiteBatRight");
	m_vecSequenceKey[1].push_back("WhiteBatDeathRight");
	m_vecSequenceKey[1].push_back("DamagedBat2Right");


	SetEndFunction<CWhiteBat>("WhiteBatDeath", this, &CWhiteBat::Death);
	SetEndFunction<CWhiteBat>("WhiteBatDeathRight", this, &CWhiteBat::Death);

	SetEndFunction<CWhiteBat>("DamagedBat2", this, &CWhiteBat::Damaged);
	SetEndFunction<CWhiteBat>("DamagedBat2Right", this, &CWhiteBat::Damaged);

	CColliderBox* Box = AddCollider<CColliderBox>("Body");

	Box->SetExtent(50.f, 50.f);
	Box->SetOffset(32.f, 20.f);
	Box->SetCollisionProfile("Monster");

	Box->SetCollisionBeginFunction<CWhiteBat>(this, &CWhiteBat::CollisionBegin);
	Box->SetCollisionEndFunction<CWhiteBat>(this, &CWhiteBat::CollisionEnd);

	return true;
}

void CWhiteBat::Update(float DeltaTime)
{
...
void CWhiteBat::PostUpdate(float DeltaTime)
{
...
void CWhiteBat::Render(HDC hDC, float DeltaTime)
{
	CCharacter::Render(hDC, DeltaTime);
}

float CWhiteBat::InflictDamage(float Damage)
{
	...
	if (m_HP <= 0)
	{
		...
		m_Scene->CreateObject<CTreasureBox>("TreasureBox", m_Pos);
...
void CWhiteBat::CollisionBegin(CCollider* Src, CCollider* Dest)
{
...
void CWhiteBat::CollisionEnd(CCollider* Src, CCollider* Dest)
{
	if (Dest->GetProfile()->Channel == ECollision_Channel::Player)
		m_PlayerOverlap = false;
}

void CWhiteBat::CollisionMouseBegin(CCollider* Src, const Vector2& MousePos)
{
}

void CWhiteBat::CollisionMouseEnd(CCollider* Src, const Vector2& MousePos)
{
}

void CWhiteBat::Death()
{
	SetActive(false);
}

void CWhiteBat::Damaged()
{
	m_Damaged = false;
}
