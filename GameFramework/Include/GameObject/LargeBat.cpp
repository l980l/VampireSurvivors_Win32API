...
CLargeBat::CLargeBat() :
	...
	m_Death(false),
	m_Damaged(false),
	m_PushBackSpeed(0.5f)
{
	SetTypeID<CLargeBat>();
}

CLargeBat::CLargeBat(const CLargeBat& Obj) :
	CMonster(Obj),
	m_Dir(Obj.m_Dir),
	m_HP(Obj.m_HP),
...
bool CLargeBat::Init()
{
	...
	AddAnimation("LargeBat");
	AddAnimation("LargeBatDeath", false, 0.3f);
	AddAnimation("DamagedLargeBat", false, 0.3f);
	AddAnimation("LargeBatRight");
	AddAnimation("LargeBatDeathRight", false, 0.3f);
	AddAnimation("DamagedLargeBatRight", false, 0.3f);

	m_vecSequenceKey[0].push_back("LargeBat");
	m_vecSequenceKey[0].push_back("LargeBatDeath");
	m_vecSequenceKey[0].push_back("DamagedLargeBat");

	m_vecSequenceKey[1].push_back("LargeBatRight");
	m_vecSequenceKey[1].push_back("LargeBatDeathRight");
	m_vecSequenceKey[1].push_back("DamagedLargeBatRight");


	SetEndFunction<CLargeBat>("LargeBatDeath", this, &CLargeBat::Death);
	SetEndFunction<CLargeBat>("LargeBatDeathRight", this, &CLargeBat::Death);

	SetEndFunction<CLargeBat>("DamagedLargeBat", this, &CLargeBat::Damaged);
	SetEndFunction<CLargeBat>("DamagedLargeBatRight", this, &CLargeBat::Damaged);

	CColliderBox* Box = AddCollider<CColliderBox>("Body");

	Box->SetExtent(110.f, 75.f);
	Box->SetOffset(100.f, 45.f);
	Box->SetCollisionProfile("Monster");

	Box->SetCollisionBeginFunction<CLargeBat>(this, &CLargeBat::CollisionBegin);
	Box->SetCollisionEndFunction<CLargeBat>(this, &CLargeBat::CollisionEnd);

	return true;
}

void CLargeBat::Update(float DeltaTime)
{
	CGameObject::Update(DeltaTime);

	m_Dir = (m_Scene->GetPlayer()->GetPos()
		- (m_Scene->GetPlayer()->GetPivot() * m_Scene->GetPlayer()->GetSize()) * 1.1f) - m_Pos;
	...
	// Player랑 겹치면 플레이어랑 반대로 가게함.
		...
		Move((m_Dir * -1.f) * 2.f / TotalMove);

	else
		MoveDir(m_Dir / TotalMove);
}

void CLargeBat::PostUpdate(float DeltaTime)
{
...
void CLargeBat::Render(HDC hDC, float DeltaTime)
{
	CCharacter::Render(hDC, DeltaTime);
}

float CLargeBat::InflictDamage(float Damage)
{
	...
	DamageNumber1->SetPos(90.f, 0.f);
	DamageNumber2->GetWidget<CDamageNumber>()->SetSize(15.f, 24.f);
	DamageNumber2->SetPos(106.f, 0.f);
...
void CLargeBat::CollisionBegin(CCollider* Src, CCollider* Dest)
{
...
void CLargeBat::CollisionEnd(CCollider* Src, CCollider* Dest)
{
	if (Dest->GetProfile()->Channel == ECollision_Channel::Player)
		m_PlayerOverlap = false;
}

void CLargeBat::CollisionMouseBegin(CCollider* Src, const Vector2& MousePos)
{
}

void CLargeBat::CollisionMouseEnd(CCollider* Src, const Vector2& MousePos)
{
}

void CLargeBat::Death()
{
	SetActive(false);

}

void CLargeBat::Damaged()
{
	m_Damaged = false;
}
