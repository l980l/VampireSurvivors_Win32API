...
{
	SetTypeID<CSkeleton2>();
}

CSkeleton2::CSkeleton2(const CSkeleton2& Obj) :
	CMonster(Obj),
	m_Dir(Obj.m_Dir),
	m_HP(Obj.m_HP),
...
bool CSkeleton2::Init()
{
	...
	AddAnimation("Skeleton2");
	AddAnimation("Skeleton2Death", false, 0.3f);
	AddAnimation("DamagedSkeleton2", false, 0.3f);
	AddAnimation("Skeleton2Right");
	AddAnimation("Skeleton2DeathRight", false, 0.3f);
	AddAnimation("DamagedSkeleton2Right", false, 0.3f);

	m_vecSequenceKey[0].push_back("Skeleton2");
	m_vecSequenceKey[0].push_back("Skeleton2Death");
	m_vecSequenceKey[0].push_back("DamagedSkeleton2");

	m_vecSequenceKey[1].push_back("Skeleton2Right");
	m_vecSequenceKey[1].push_back("Skeleton2DeathRight");
	m_vecSequenceKey[1].push_back("DamagedSkeleton2Right");


	SetEndFunction<CSkeleton2>("Skeleton2Death", this, &CSkeleton2::Death);
	SetEndFunction<CSkeleton2>("Skeleton2DeathRight", this, &CSkeleton2::Death);

	SetEndFunction<CSkeleton2>("DamagedSkeleton2", this, &CSkeleton2::Damaged);
	SetEndFunction<CSkeleton2>("DamagedSkeleton2Right", this, &CSkeleton2::Damaged);

	CColliderBox* Box = AddCollider<CColliderBox>("Body");

	Box->SetExtent(75.f, 80.f);
	Box->SetOffset(55.f, 45.f);
	Box->SetCollisionProfile("Monster");

	Box->SetCollisionBeginFunction<CSkeleton2>(this, &CSkeleton2::CollisionBegin);
	Box->SetCollisionEndFunction<CSkeleton2>(this, &CSkeleton2::CollisionEnd);

	return true;
}

void CSkeleton2::Update(float DeltaTime)
{
...
void CSkeleton2::PostUpdate(float DeltaTime)
{
...
void CSkeleton2::Render(HDC hDC, float DeltaTime)
{
	CCharacter::Render(hDC, DeltaTime);
}

float CSkeleton2::InflictDamage(float Damage)
{
...
void CSkeleton2::CollisionBegin(CCollider* Src, CCollider* Dest)
{
...
void CSkeleton2::CollisionEnd(CCollider* Src, CCollider* Dest)
{
	if (Dest->GetProfile()->Channel == ECollision_Channel::Player)
		m_PlayerOverlap = false;
}

void CSkeleton2::CollisionMouseBegin(CCollider* Src, const Vector2& MousePos)
{
}

void CSkeleton2::CollisionMouseEnd(CCollider* Src, const Vector2& MousePos)
{
}

void CSkeleton2::Death()
{
	SetActive(false);

}

void CSkeleton2::Damaged()
{
	m_Damaged = false;
}
